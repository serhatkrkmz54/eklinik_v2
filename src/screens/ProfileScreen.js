        ...
        >
          <View style={styles.header}>
            ...
            <View style={styles.profileSection}>
              <View style={[styles.profileImageContainer, userData?.patientProfile?.hasChronicIllness && styles.chronicIllnessBorder]}>
                <Image
            ...
            <View style={styles.statsContainer}>
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>{formatDate(userData?.patientProfile?.dateOfBirth)}</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <MaterialIcons name="height" size={24} color="#fff" />
                <Text style={styles.statLabel}>Boy/Kilo</Text>
                <Text style={styles.statValue}>{userData?.patientProfile?.height ? `${userData.patientProfile.height}cm` : '--'}/{userData?.patientProfile?.weight ? `${userData.patientProfile.weight}kg` : '--'}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.detailsContainer}>
            ...
            {/* GÜNCELLENDİ: Adres alanı İletişim Bilgileri altına taşındı ve eklendi */}
            {userData?.patientProfile?.address && (
                <View style={styles.infoRow}>
                  <MaterialIcons name="location-pin" size={24} color="#008B8B" />
                  <View style={styles.infoTextContainer}>
                    <Text style={styles.infoLabel}>Adres</Text>
                    <Text style={styles.infoValue}>{userData.patientProfile.address}</Text>
                  </View>
                </View>
            )}
            ...
            {(userData?.patientProfile?.birthPlaceCity || userData?.patientProfile?.birthPlaceDistrict) && (
                <View style={styles.infoRow}>
                  <MaterialIcons name="location-city" size={24} color="#008B8B" />
                  <View style={styles.infoTextContainer}>
                    <Text style={styles.infoLabel}>Doğum Yeri</Text>
                    <Text style={styles.infoValue}>
                      {userData.patientProfile.birthPlaceCity || ''}
                      {userData.patientProfile.birthPlaceCity && userData.patientProfile.birthPlaceDistrict ? ' / ' : ''}
                      {userData.patientProfile.birthPlaceDistrict || ''}
                    </Text>
                  </View>
                </View>
            )}
            {userData?.patientProfile?.country && (
                <View style={styles.infoRow}>
                  <MaterialIcons name="public" size={24} color="#008B8B" />
                  <View style={styles.infoTextContainer}>
                    <Text style={styles.infoLabel}>Ülke</Text>
                    <Text style={styles.infoValue}>{userData.patientProfile.country}</Text>
                  </View>
                </View>
            )}
            {userData?.patientProfile?.hasChronicIllness !== null && (
                <View style={[styles.infoRow, userData.patientProfile.hasChronicIllness && styles.chronicIllnessRow]}>
                  <MaterialIcons name="healing" size={24} color={userData.patientProfile.hasChronicIllness ? '#FF4B55' : '#008B8B'} />
                  <View style={styles.infoTextContainer}>
                    <Text style={styles.infoLabel}>Kronik Hastalık</Text>
                    <Text style={styles.infoValue}>{userData.patientProfile.hasChronicIllness ? 'Var' : 'Yok'}</Text>
                  </View>
                </View>
            )}
            {userData?.patientProfile?.isMedicationDependent !== null && (
                <View style={styles.infoRow}>
                  <MaterialIcons name="medication" size={24} color="#008B8B" />
                  <View style={styles.infoTextContainer}>
                    <Text style={styles.infoLabel}>Sürekli İlaç Kullanımı</Text>
                    <Text style={styles.infoValue}>{userData.patientProfile.isMedicationDependent ? 'Var' : 'Yok'}</Text>
                  </View>
                </View>
            )}
